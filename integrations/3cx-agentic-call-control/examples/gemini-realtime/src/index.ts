...
async function main() {
    ...
    if (!appconfig.geminiApiKey) {
        console.error(chalk.red('Error: GOOGLE_API_KEY is missing from the project .env'));
        process.exit(1);
    }

    if (!appconfig.appId || !appconfig.appSecret || !appconfig.pbxBase) {
        console.error(chalk.red(
            'Error: THREECX_APP_ID, THREECX_APP_SECRET, and THREECX_PBX_BASE_URL '
            + 'must be set in the project .env',
        ));
        process.exit(1);
    }
